...
describe('Correctly resolve path of the view', () => {
    test('when theme and this library both have the view file', () => {
        expect(view.require.resolve('comment/disqus'))
            .toBe(path.join(themeDir, 'layout/comment/disqus.jsx'));
    });

    test('when only this library has the view file', () => {
        expect(view.require.resolve('misc/open_graph'))
            .toBe(path.join(__dirname, '../view/misc/open_graph.js'));
    });

    test('when only the theme has the view file', () => {
        expect(view.require.resolve('example'))
            .toBe(path.join(themeDir, 'layout/example.jsx'));
    });
...
describe('Correctly require() the view', () => {
    ...
    test('don\' resolve absolute path', () => {
        expect(view.require(path.join(__dirname, '../view/comment/disqus')))
            .toBe(DisqusLibraryView);
    });
});
